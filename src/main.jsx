...
function SettingsPage({settings,updateSetting}){
  const Row=({id,title,desc})=>
    <div className="settingrow">
      <div><b>{title}</b><small>{desc}</small></div>
      <button className={settings[id]?"toggle on":"toggle"} onClick={()=>updateSetting(id,!settings[id])}>
        <i/>{settings[id]?"ON":"OFF"}
      </button>
    </div>;

  const themes=[
    {id:"lime",label:"Lime",desc:"TradeMindMZ default"},
    {id:"blue",label:"Blue",desc:"Cool terminal"},
    {id:"purple",label:"Purple",desc:"AI focus"},
    {id:"red",label:"Red",desc:"High alert"},
  ];

  return <div className="settingspage">
    <div className="settingshero">
      <div>
        <label><Settings/> AI CONTROL CENTER</label>
        <h1>Control your AI usage.</h1>
        <p>Turn AI providers on or off to control analysis and API usage. These settings are saved on this device.</p>
      </div>
    </div>

    <div className="panel settingspanel tmz-theme-panel">
      <div className="settinghead">
        <div>
          <h2>APPEARANCE</h2>
          <p>Choose the terminal accent without changing trading logic.</p>
        </div>
        <span className="status on"><i/>THEME</span>
      </div>
      <div className="tmz-theme-grid">
        {themes.map(theme=>(
          <button
            type="button"
            key={theme.id}
            className={`tmz-theme-option ${(settings.theme||"lime")===theme.id?"active":""}`}
            data-theme-option={theme.id}
            onClick={()=>updateSetting("theme",theme.id)}
          >
            <span className="tmz-theme-swatch"/>
    ...
    <div className="panel settingspanel">
      <div className="settinghead">
        <div><h2>AI ENGINE</h2><p>Main controls for TradeMindMZ intelligence.</p></div>
        <span className={settings.ai?"status on":"status"}><i/>{settings.ai?"AI ACTIVE":"AI DISABLED"}</span>
      </div>
      <Row id="ai" title="AI Analysis" desc="Master switch for AI analysis."/>
      <Row id="openai" title="OpenAI" desc="Allow OpenAI to perform analysis."/>
      <Row id="groq" title="Groq" desc="Allow Groq to perform analysis and fallback."/>
    ...
    <div className="panel settingspanel">
      <div className="settinghead">
        ...
        <span className={settings.pushNotifications?"status on":"status"}><i/>{settings.pushNotifications?"ALERTS ON":"ALERTS OFF"}</span>
      </div>
      <Row id="pushNotifications" title="Qualified Trade Notifications" desc="Receive a phone notification when TradeMindMZ finds a qualified trade." />
    </div>

    <div className="panel costpanel">
      <h3>AI COST CONTROL</h3>
      <p>When AI Analysis is OFF, TradeMindMZ must not send AI analysis requests. Market data and historical collection can continue independently.</p>
      <div className="costgrid">
        <span><b>{settings.ai?"ACTIVE":"OFF"}</b><small>AI Analysis</small></span>
